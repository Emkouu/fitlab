...
# Изрично, а не каквото е останало в извикващия shell. Веднъж деплой беше
# пуснат от сесия с `umask 077`; билдът записа .next/static като drwx------,
# nginx (www-data) не можеше да я чете и всеки CSS/JS връщаше 403. Страницата
# се рендираше от сървъра, но оставаше без стил и без хидратация.
umask 022
...
ls -ld /opt/fitlab/app/.next/static              # трябва да е drwxr-xr-x — вж. по-долу
df -h /                                          # диск (билдовете трупат .next/cache)
free -m                                          # RAM
```

**Сайтът е без стил, а страниците се зареждат.** Симптом: HTML-ът идва (200),
но всеки `/_next/static/*` връща **403**, така че нито CSS, нито JS стигат до
браузъра — страницата остава сървърно рендирана, без хидратация. Причината е
права: `.next/static` е записана `drwx------` и `www-data` не може да я чете.
Случва се, когато билдът е пуснат от shell с ограничаващ `umask`.

```bash
chmod -R a+rX /opt/fitlab/app/.next /opt/fitlab/app/public
```

`deploy.sh` вече задава `umask 022` изрично (§11), така че това не бива да се
повтори при деплой през него.
