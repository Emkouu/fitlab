...
| Потребители | `emkou` (admin, 2 web домейна, ~57 GB) и `Hyperlink` (admin, без домейни) |
...
⚠️ **Диск:** `emkou` държи ~57 GB. Провери свободното място с `df -h /` преди
първия билд — `node_modules` + `.next` са около 1,5–2 GB, а всеки деплой оставя
кеш. Ако свободното е под ~20 GB, изчисти преди да започнеш (§Приложение А).

Бележка встрани: `Hyperlink` е втори **admin** акаунт без домейни. Ако не се
ползва, свали го до роля `user` или го изтрий — админ акаунт с достъп до целия
панел е ненужна повърхност за атака. Не е блокер за тази миграция.
