...
- **Neither gateway carries a certificate for the public host we dial**, and both chain to the bank's private CA. So `client.ts` does two things, never `rejectUnauthorized: false`:
  - pins the bank's CA — `FIBANK_CA_PEM` in `fibankCa.ts` (`CS ROOT` + `LOCAL CA`, appended to `tls.rootCertificates`). `CS ROOT` is the one the bank shipped inside our keystore, so the anchor arrived out of band; `LOCAL CA` is trusted only because it verifiably signed that `CS ROOT`. Public certs, kept in the repo — not in env, so the server can't be talked into a different anchor.
  - checks the hostname against the name each gateway really presents — `isFibankTestCertificate()` / `isFibankProductionCertificate()` in `protocol.ts` (`eur-3ds-ecomm-test.int.fibank.bg`, `3ds-v2-ecomm-prod.int.fibank.bg`). Exact per-name match; anything else gets Node's error unchanged.
  - The bank has been asked for certificates valid for `mdpay.fibank.bg` / `mdpay-test.fibank.bg`; when they arrive both name lists can go.
