...
- **`refundPaymentAction` („Върни сумата")** is the per-transaction refund, sitting on the transaction itself in `/admin/payments` and in the client's „Картови транзакции" block. It exists because the acquirer's requests name a `TrnID` („пълно възстановяване на сумата на двете транзакции"), while `refundDepositAction` answers the different question „the client wants their standing deposit back" and is therefore gated on `User.depositBalance` — invisible exactly when the deposit has been burned, cleared, or belongs to a cancelled booking. The button appears per `isRefundable()` (tested): current attempt, has an `ecommTransId`, `Payment.status = paid`, not yet refunded — superseded attempts never charged anything and `command=k` only knows the current transaction. A `pending` row needs „Провери в банката" first. super_admin only; refunds the **full** `Payment.amount`, then takes off `depositBalance` however much of it is still standing (clamped at 0). The booking is deliberately left alone — cancelling it is a separate, audited call.
