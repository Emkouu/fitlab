...
- **`refundPaymentAction` („Възстанови сумата")** is the per-transaction refund, sitting on the transaction itself in `/admin/payments` and in the client's „Картови транзакции" block. It exists because the acquirer's requests name a `TrnID` („пълно възстановяване на сумата на двете транзакции"), while `refundDepositAction` answers the different question „the client wants their standing deposit back" and is therefore gated on `User.depositBalance` — invisible exactly when the deposit has been burned, cleared, or belongs to a cancelled booking.
  - **The button is unconditional and the refund is forced.** Gating it on `Payment.status = paid` would hide it precisely where it is needed: an abandoned card page leaves the row `pending` with no `RESULT` while the money moved, and those are the rows the bank writes about. Our record is the unreliable half, so `command=k` goes out for any attempt staff point at and the bank answers; a refusal is shown verbatim (`formatResultCode`). `isRefundable()` (tested) no longer decides whether the button exists — only whether the press is routine or **forced**, which drives a warning above the two-step confirm.
  - `refundCardPayment` takes `transId` (which attempt to reverse — a retried card leaves several behind one row and the bank often quotes a superseded one) and `force` (skip the local status guard). Ownership of `transId` is verified against the row, so an id from another payment is refused rather than sent to the bank. Automatic callers (class cancel, deposit refund) keep the guarded default.
  - Refunding the **current** attempt writes the row's `status`/`ecommRefundTransId`/`refundedAmount`/`refundedAt` and takes off `depositBalance` however much is still standing (clamped at 0). Refunding a **superseded** attempt records the refund inside its `ecommHistory` entry instead — the row's own refund columns describe the current attempt — and never touches the balance, since a declined attempt never put a deposit there. The booking is deliberately left alone either way; cancelling it is a separate, audited call.
