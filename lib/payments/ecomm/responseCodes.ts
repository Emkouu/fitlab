/**
 * ECOMM `RESULT_CODE` values, from the acquirer's `ResponseCodes 2022.txt`.
 *
 * These are **informational only** — the integration manual §4.2 is explicit
 * that success or failure is decided by `RESULT` alone, and nothing in the
 * booking flow branches on a code from here. Their job is to make a stored
 * payment legible to a human: „116" in a log or an admin screen means nothing,
 * „Decline, not sufficient funds" tells staff what to say to the client.
 *
 * Generated from the bank's file; keep it in sync if they send a newer one.
 */
...
const RESPONSE_CODES: Record<string, string> = {
  ...
  "160": "Decline, require Strong Customer Authentication",
  "164": "Decline, Fraud/Security violation",
  "180": "Decline, by cardholders wish",
  "182": "Decline, Card is not active",
...
/**
 * Human-readable reason for a `RESULT_CODE`, or null for an unknown code.
 *
 * The bank may add codes without telling us, so an unrecognised value is not an
 * error — the caller falls back to showing the raw code.
 */
export function describeResultCode(code: string | null | undefined): string | null {
  if (!code) return null;
  return RESPONSE_CODES[code.trim()] ?? null;
}

/** `"116 — Decline, not sufficient funds"`, or just the code when unknown. */
export function formatResultCode(code: string | null | undefined): string | null {
  if (!code) return null;
  const trimmed = code.trim();
  const description = describeResultCode(trimmed);
  return description ? `${trimmed} — ${description}` : trimmed;
}

/** Approvals are the 0xx family (`000` = approved). Informational only. */
export function isApprovalCode(code: string | null | undefined): boolean {
  if (!code) return false;
  return /^0\d\d$/.test(code.trim());
}
